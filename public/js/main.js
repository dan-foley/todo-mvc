...
async function editTodo() {
    ...
    const span = li.querySelector('span.not, span.completed'); // existing text
    ...
    const saveEdit = async () => {
        ...
        if (newText && newText !== oldText) {
            try {
                ...
                const data = await response.json();
                console.log(data); // "Todo Edited"
                location.reload();
            } catch (err) {
                console.error(err);
            }
        } else {
            input.replaceWith(span); // restore original if empty/unchanged
        }
    };
